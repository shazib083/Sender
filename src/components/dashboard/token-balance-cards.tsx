...
const DISPLAY_TOKENS: TokenSymbol[] = ["USDC", "EURC", "ETH"];
...
function BalanceCard({ symbol, balance, isLoading, onRefresh, isRefreshing }: BalanceCardProps) {
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-surface-300 bg-surface-100 p-5 transition-all hover:border-brand-500/40 hover:shadow-glow-sm">
      ...
      <div className="relative flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-wider text-gray-500">{symbol}</p>
            {isLoading ? (
